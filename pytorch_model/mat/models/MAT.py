...
# from model.cnn.mat.utils import cont_grad
...
class MAT(nn.Module):
    def __init__(self, net='xception',feature_layer='b3',attention_layer='final',num_classes=1, M=8,mid_dims=256,\
    dropout_rate=0.5,drop_final_rate=0.5, pretrained=False,alpha=0.05,size=(256,256),margin=1,inner_margin=[0.01,0.02]):
        ...
        self.ensemble_classifier_fc=nn.Sequential(nn.Linear(mid_dims*2,mid_dims),nn.Hardswish(),nn.Linear(mid_dims,num_classes),nn.Sigmoid())
    ...
    def forward(self, x):
        ...
        feature_matrix=torch.cat((feature_matrix,projected_final),1)
        ensemble_logit=self.ensemble_classifier_fc(feature_matrix)
        # print("ensemble_logit : ",ensemble_logit)
